...
#		self.differential_decoder = gr.diff_decoder_bb(2)
		...
		# change rate to 57kHz, filter and convert to complex
		self.resample = blks2.rational_resampler_fff(demod_rate, int(57e3))
# FIXME add resample filter
		self.f2c = gr.float_to_complex()
		self.connect (self.rds_filter, self.resample, self.f2c)

		# AGC
		scale = (1.0/16384.0)
		self.pre_scaler = gr.multiply_const_cc(scale)	# NRZ
		self.agc = gr.feedforward_agc_cc(16, 2.0)
		self.connect (self.f2c, self.pre_scaler, self.agc)

		# symbol clock recovery using a costas loop
		M = 2				# modulation order
		theta = 0			# phase correction
		alpha = 0.01			# gain
		beta = 0.25*alpha*alpha		# gain
		fmax = 0.1
		fmin = -fmax
		mu = 0.5			# initial interpolator value
		gain_mu = 0.1
		omega = 48			# samples per symbol
		gain_omega = 0#0.25*gain_mu*gain_mu
		omega_rel = 0#0.005
		self.clock_recov = gr.mpsk_receiver_cc(M, theta, alpha, beta,
			fmin, fmax, mu, gain_mu, omega, gain_omega, omega_rel)
		self.connect(self.agc, self.clock_recov)

		# diff_decode, slice
		self.diff_decode = gr.diff_phasor_cc()
		self.slicer = gr.constellation_decoder_cb([1, -1], [0, 1])
		self.connect (self.clock_recov, self.diff_decode, self.slicer)

		self.msgq = gr.msg_queue()
		self.rds_sink = rds.data_sink(self.msgq)
...
#		self.connect(self.differential_decoder, self.rds_decoder)
		self.connect (self.slicer, self.rds_sink)
...
####################### GUI ################################
	...
	def _build_gui(self, vbox, demod_rate, audio_rate):
		...
		if 0:
			self.src_fft = fftsink2.fft_sink_c (self.panel, title="pilot carrier",
											fft_size=512, sample_rate=demod_rate)
			self.connect (self.guts.stereo_carrier_filter, self.src_fft)
